...
four functions (designed for both models with weighted and binary coefficients) implement the MCMC steps of the MCMC-SAEM.
[there is some variants with _adapt when some parameter are learned with adaptations, 
these functions are useful only for verifying the codes not for the final experiments]
- mh        : performs MCMC the steps of the MCMC-SAEM algorithm based on Metropolis-Hastings transitions
- slice_rieman        : performs MCMC the steps of the MCMC-SAEM algorithm based on GSS transitions
