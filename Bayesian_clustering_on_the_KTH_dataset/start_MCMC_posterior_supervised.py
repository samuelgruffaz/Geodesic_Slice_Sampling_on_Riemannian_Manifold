...
# This script should be run after "start_EM_supervised.py"
...
for env in ["d1","d2","d3","d4"]:
    ...
    print("open")
    # we open the results of the EM algorithm
    with open(os.path.join(path_save,'supervised_diag_latent'+str(d_latent2)+'_niter'+str(n_iter)+"clustering_est_"+env+".pkl"),'rb') as f:
        tup=pickle.load(f)
    (log_pi,F1,mu,cov,F3,Z,Z_test,ind_train,ind_test)=tup

    # we computes the cluster assigments weights related to MCMC samples
    weights_mean,weights_std,weights=Bayesian_weights_test(features_train_U,features_train_Sigma,features_train_A,features_test_U,features_test_Sigma,features_test_A,log_pi,F1,mu,cov,F3,Z,n_mcmc=1000,n_burn=10)
    print("save")
    # We save these weights
    with open(os.path.join(path_save,'weights_supervised_diag_latent'+str(d_latent2)+'_niter'+str(n_iter)+"clustering_est_"+env+".pkl"),'wb') as f:
        pickle.dump(weights,f)
    Label_est_new=predict_label_with_Z(weights_mean)
    print("score with mcmc",score(Label_est_new,Label_test))
