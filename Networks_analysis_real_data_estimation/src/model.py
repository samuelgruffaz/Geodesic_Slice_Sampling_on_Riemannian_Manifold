...
def IS_lk(As,theta,Init=None,n_ss=100,stride=10,burn=500):
    ...
    for i in range(1,n_ss):
        lk=np.logaddexp(lk,log_ratio(Xss[:,i,:,:],lss[:,i,:],As,F_chaps,muxs[:,0,:],theta))
...
def log_ratio(Xs,ls,As,F_chaps,muxs,theta):
    ...
    for i in range(n_samples):
        ...
        den+=-spa.log_vmf(F_chaps[i])+sigma_p_inv_squarre*np.linalg.norm(ls-muxs[i])+(F_chaps[i]*Xs[i]).sum()+p*np.log(sigma_p_inv_squarre)/2

    # (As | Xs, ls, sigma)
    Xs_comp = st.comp(Xs, ls)
    nom= -0.5*((As-Xs_comp)**2).sum()/sigma**2 - n_samples*(n**2)*np.log(sigma)
    nom += -n_samples*spa.log_vmf(F)
    
    # (ls | mu, sigma_l)
    p = F.shape[1]
    nom += -0.5*((ls-mu[None,:])**2).sum()/sigma_l**2 - n_samples*p*np.log(sigma_l)
