...
def mcmc_saem(As, Xs_mh, ls_mh, theta, n_iter=100, mala=False,slice_rieman=False,mh_geo=False, prop_X=0.01, prop_l=0.5, n_mcmc=20, history=True, setting="gaussian",use_adapt=True,m=1,likelihood=False):
    ...
    print("rate",rate_X)
    if likelihood:
        lk_true=model.IS_lk(As,theta,Init=Xs_mh,n_ss=5000,stride=4,burn=1000)
    else:
        lk_true="non"
    result = {
        "theta": theta,
